...
.cookie
	...
	move.l	a2,a3					; Source brick struct
	move.l 	GAMESCREEN_BackPtr(a5),a4	; Set up background
	move.l	d3,d6
	mulu.w	#(RL_SIZE*4),d6			; TODO: dynamic handling of no. of bitplanes if needed
	add.l	d2,d6					; Add byte (x pos) to longword (y pos)
	add.l	d6,a4

	move.l	GAMESCREEN_Ptr(a5),a2	; Set up destination
	add.l	d6,a2

	bsr		CookieBlitBrickToScreen
									; Then copy to backscreen
	exg		a2,a0					; Source
	exg		a4,a1					; Destination
	moveq	#DEFAULT_MASK,d0
	moveq	#RL_SIZE-4,d1
	move.w	#(64*8*4)+2,d2
	bsr		CopyBlit
